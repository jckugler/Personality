...
function PersonalityMap({
  ...
  return (
    <div className="relative h-[500px] overflow-hidden bg-[#f4f5f4] text-[#6f7474] sm:h-[620px]">
      ...
      <AxisLabel className="left-4 top-1/2 -translate-y-1/2 text-right sm:left-8" label={"Introvert\nPassive\nReserved"} />
      <AxisLabel className="right-4 top-1/2 -translate-y-1/2 text-left sm:right-8" label={"Extrovert\nActive\nImplementor"} />

      <div className="absolute inset-[86px_62px_78px_62px] sm:inset-[104px_118px_92px_118px]">
      ...
      {data.length ? (
        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center text-center text-charcoal/60">
          <div>
...
function Quadrant({ color, className }: { color: StyleColor; className: string }) {
  ...
  return (
    <div className={`absolute h-[47%] w-[47%] ${content.className} ${className}`}>
      ...
      {content.words.map((word, index) => (
        <span
          key={word}
          className="absolute z-10 text-[9px] font-black uppercase tracking-wide text-charcoal/55 sm:text-[11px]"
...
function AxisArrow({ direction }: { direction: "horizontal" | "vertical" }) {
  if (direction === "horizontal") {
    return (
      <div className="absolute left-[15%] right-[15%] top-1/2 z-10 h-0.5 -translate-y-1/2 bg-[#7b8080]">
        <span className="absolute -left-1.5 top-1/2 h-3 w-3 -translate-y-1/2 rotate-45 border-b-2 border-l-2 border-[#7b8080]" />
        <span className="absolute -right-1.5 top-1/2 h-3 w-3 -translate-y-1/2 rotate-45 border-r-2 border-t-2 border-[#7b8080]" />
      </div>
    );
  }

  return (
    <div className="absolute bottom-[12%] left-1/2 top-[14%] z-10 w-0.5 -translate-x-1/2 bg-[#7b8080]">
      <span className="absolute -top-1.5 left-1/2 h-3 w-3 -translate-x-1/2 rotate-45 border-l-2 border-t-2 border-[#7b8080]" />
      <span className="absolute -bottom-1.5 left-1/2 h-3 w-3 -translate-x-1/2 rotate-45 border-b-2 border-r-2 border-[#7b8080]" />
    </div>
  );
}
