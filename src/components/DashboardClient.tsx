...
function ScoreMapV3({
  ...
  const mapHeight = 700;
  const nameScale = Math.max(0.38, Math.min(1, 1 / view.scale));
  const nameFontSize = Math.max(5, Math.round(11 * nameScale));
  const namePaddingY = Math.max(1, Math.round(3 * nameScale));
  const namePaddingX = Math.max(2, Math.round(7 * nameScale));
  ...
  function zoomTo(nextScale: number, clientX?: number, clientY?: number) {
    setView((current) => {
      ...
      const centerX = (clientX ?? rect.left + rect.width / 2) - rect.left;
      const centerY = (clientY ?? rect.top + rect.height / 2) - rect.top;
  ...
  function handleWheel(event: React.WheelEvent<HTMLDivElement>) {
    ...
    zoomTo(view.scale + direction, event.clientX, event.clientY);
  }
  ...
  return (
    ...
    >
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs font-bold text-white/40">Zoom {Math.round(view.scale * 100)}%</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => zoomTo(view.scale - 0.15)}
      ...
      <div
        ...
        className="relative h-[calc(100vh-260px)] min-h-[560px] overflow-hidden"
        style={{
          background:
            ...
            data.map((point) => (
              <button
                ...
                style={{
                  ...
                  minWidth: Math.max(14, Math.round(34 * nameScale)),
                  borderRadius: 4,
                  fontSize: nameFontSize,
                  lineHeight: `${Math.max(7, Math.round(14 * nameScale))}px`,
                  fontWeight: 900,
...
function mapPositionFromScores(participant: {
  ...
  const spreadX = spreadCoordinate(x);
  const spreadY = spreadCoordinate(y);

  return {
    left: clampMapPercent(50 + spreadX * 0.38),
    top: clampMapPercent(50 - spreadY * 0.38)
  };
}
...
function spreadCoordinate(value: number) {
  const direction = value < 0 ? -1 : 1;
  const amount = Math.min(100, Math.abs(value));
  return direction * Math.pow(amount / 100, 0.78) * 100;
}

function clampZoom(value: number) {
  return Math.max(0.7, Math.min(2.5, Number(value.toFixed(2))));
}
