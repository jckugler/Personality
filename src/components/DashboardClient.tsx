        ...
        <section className="border border-white/10 bg-white/[0.04] p-4 shadow-dark sm:p-5">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs font-black uppercase tracking-[0.22em] text-white/35">Map version: dark zoom score-placement-v2</p>
            </div>
            <button
              type="button"
              onClick={refresh}
...
function PersonalityMap({
  ...
  return (
    <div className="border border-white/10 p-3" style={{ background: "#070807" }}>
      ...
      <div
        className="h-[520px] overflow-auto sm:h-[680px]"
        style={{
          background:
            "linear-gradient(rgba(255,255,255,0.035) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.035) 1px, transparent 1px), #101312",
...
function Quadrant({ color, className }: { color: StyleColor; className: string }) {
  ...
  return (
    <div
      className={`absolute h-[47%] w-[47%] ${className}`}
      style={{
        background: content.color,
        boxShadow: "0 0 0 1px rgba(255,255,255,0.1), 0 18px 35px rgba(0,0,0,0.22)"
      }}
    >
...
function mapPositionFromScores(participant: {
  ...
  const scores = {
    red: Number(participant.red_score),
    yellow: Number(participant.yellow_score),
    green: Number(participant.green_score),
    blue: Number(participant.blue_score)
  };
  const total = Math.max(scores.red + scores.yellow + scores.green + scores.blue, 1);
  const x = ((scores.red + scores.yellow - scores.green - scores.blue) / total) * 100;
  const y = ((scores.red + scores.blue - scores.yellow - scores.green) / total) * 100;
